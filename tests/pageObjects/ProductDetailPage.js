...
class ProductDetailPage {
  constructor(page) {
    ...
    // Selectors based on MCP server analysis - FIXED
    // Fix: Use specific selectors for mobile/desktop h1 elements to avoid strict mode violations
    this.productTitle = page.locator("h1.hidden.md\\:flex"); // Desktop version (visible on desktop)
    this.productTitleMobile = page.locator("h1.md\\:hidden"); // Mobile version (hidden on desktop)
    ...
    // Fix: Quantity inputs are actually type="text" with max attribute, not type="number"
    this.quantityInput = page.locator('input[max="999999999"]');
  ...
  async getProductTitle() {
    // Try desktop version first, then mobile version
    try {
      if (await this.productTitle.isVisible()) {
        return await this.productTitle.textContent();
      } else {
        return await this.productTitleMobile.textContent();
      }
    } catch (error) {
      // Fallback to any h1 if specific selectors fail
  ...
  async verifyProductTitle(expectedTitle) {
    // Check visibility first to avoid strict mode violations
    const isDesktopVisible = await this.productTitle.isVisible();
    const isMobileVisible = await this.productTitleMobile.isVisible();
    
    if (isDesktopVisible) {
      await expect(this.productTitle).toBeVisible();
      await expect(this.productTitle).toHaveText(expectedTitle);
    } else if (isMobileVisible) {
      await expect(this.productTitleMobile).toBeVisible();
      await expect(this.productTitleMobile).toHaveText(expectedTitle);
    } else {
      // Fallback - at least one h1 should be visible
      await expect(this.page.locator("h1").first()).toBeVisible();
      await expect(this.page.locator("h1").first()).toHaveText(expectedTitle);
    }
