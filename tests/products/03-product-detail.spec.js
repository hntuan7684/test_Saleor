...
test.describe("Product Detail Page Tests", () => {
  ...
  test("PD001 - Verify Product Title is displayed correctly", async () => {
    ...
    // Use the updated page object method that handles mobile/desktop properly
    await pd.verifyProductTitle("BELLA + CANVAS - Jersey Tee - 3001");
  });
  ...
  test("PD007 - Verify Quantity selector is working correctly", async () => {
    // Use the updated selector from page object (fixed to use correct selector)
    const count = await pd.quantityInput.count();
    
    if (count > 0) {
      const qty = pd.quantityInput.first();
      await qty.fill("2");
  ...
  test('PD019 - Verify product information accuracy', async () => {
    ...
    await pd.page.waitForLoadState("networkidle", { timeout: 30000 });
    
    // Verify product title - use .first() to handle multiple h1 elements
    const title = await pd.page.locator('h1').first().textContent();
    ...
    // Verify description content - find the paragraph with actual content
    const proseBlock = pd.page.locator('.prose').first();
    await expect(proseBlock).toBeVisible({ timeout: 10000 });
    
    const descriptionParagraphs = proseBlock.locator("p");
    const count = await descriptionParagraphs.count();
    expect(count).toBeGreaterThan(0);
    
    // Find the paragraph with actual content (skip empty ones)
    let description = '';
    for (let i = 0; i < count; i++) {
      const text = await descriptionParagraphs.nth(i).textContent();
    ...
    expect(description).toContain('Jersey Tee');
  });

  test('PD020 - Verify responsive behavior', async ({ page }) => {
    // Test mobile viewport
    await page.setViewportSize({ width: 375, height: 812 });
    await expect(page.locator('h1.md\\:hidden')).toBeVisible(); // Mobile h1
    await expect(page.locator('div[data-swiper-slide-index] img').first()).toBeVisible();
    
    // Test tablet viewport
    await page.setViewportSize({ width: 768, height: 1024 });
    await expect(page.locator('h1.hidden.md\\:flex')).toBeVisible(); // Desktop h1
    await expect(page.locator('.price-main')).toBeVisible();
