...
Then('I should see validation errors', async function() {
  const validationErrors = this.page.locator('[data-testid="validation-errors"]');
...
Then('I should see a clear error message', async function() {
  const clearError = this.page.locator('[data-testid="clear-error"]');
  await expect(clearError).toBeVisible();
});
