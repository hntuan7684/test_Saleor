...
When('I select size {string}', async function(size) {
  ...
  if (!sizeButton) {
    // This product might not have size variants, skip the test
    console.log(`Size "${size}" not available on this product - skipping size selection`);
    return;
  }
...
When('I select a different color option', async function() {
  ...
  // Wait for color buttons to be available with better selector
  const colorButtons = this.page.locator('button[title]').filter({ hasText: /-$/ });
  await colorButtons.first().waitFor({ timeout: 10000 });
  ...
  expect(count).toBeGreaterThan(0);
  await colorButtons.nth(Math.min(1, count - 1)).click();
...
Then('all product elements should be properly displayed', { timeout: 15000 }, async function() {
  // Check mobile h1 element (should be visible on mobile)
  const mobileH1 = this.page.locator("h1.md\\:hidden");
  await expect(mobileH1).toBeVisible();
...
When('I enter quantity {string}', { timeout: 15000 }, async function(quantity) {
  // Check if quantity input exists on this product
  const qtySelectors = [
    'input[type="number"]',
    'input[name*="quantity"]',
  ...
  for (const selector of qtySelectors) {
    ...
    if (await element.count() > 0) {
      qtyInput = element.first();
      break;
    }
  }
  
  if (!qtyInput) {
    console.log('Quantity input not available on this product - skipping quantity test');
    return;
  }
...
Then('the quantity should be set to {string}', async function(expectedQuantity) {
  ...
  if (count === 0) {
    console.log('Quantity input not available - skipping validation');
    return;
  }
...
Then('the product should display title, price, and description', async function() {
  ...
  await expect(this.page.locator("h1.hidden.md\\:flex")).toBeVisible();
